jQuery.fn.truncate = function (len,_this) {
    if (_this.val().length > len) {
    	var _v = _this.val().substring(0, len);
	    _this.val(_v);
        AuthoringTool.Common.showWarningMessage("You can only type in " + len + " characters in the text area, the text you typed will be truncated.");
    }
    return false;
}
...
jQuery.fn.maxLength = function (len) {
    ...
    var handleKeyPress = function (e) {
        ...
        if (val.length == len && selected < 1)
            ...
            return textarea.truncate(len,textarea);
    };
    ...
    var handleChange = function (e) {
        ...
        textarea.truncate(len,textarea);
    };
...
$(function () {
    ...
    $('textarea[maxlength]').maxLength(null, false);
});